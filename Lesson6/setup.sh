...
stmt="STOP SLAVE;"
docker exec mysql_master sh -c "mysql -u root -p"${ROOT_PASSWORD}" -e '$stmt'"
stmt="STOP SLAVE;"
docker exec mysql_slave sh -c "mysql -u root -p"${ROOT_PASSWORD}" -e '$stmt'"
...
auth_master_stmt="CHANGE MASTER TO GET_MASTER_PUBLIC_KEY=1;"
docker exec mysql_slave sh -c "mysql -u root -p${ROOT_PASSWORD} -e '$auth_master_stmt'"
